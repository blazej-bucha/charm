...
int main(void)
{
    ...
    charmf_shc *shcs = charmf_shc_init(nmax, 1.0, 1.0);
    if (shcs == NULL)
    {
    ...
    charmf_shc_read_gfc(fptr, nmax, shcs, err);
    fclose(fptr);
    ...
    charmf_shc_write_bin(shcs, nmax, fptr, err);
    fclose(fptr);
     ...
     * called "shcs2" */
    charmf_shc *shcs2 = charmf_shc_init(nmax, 1.0, 1.0);
    if (shcs2 == NULL)
    {
        fprintf(stderr, "Failed to initialize the \"charmf_shc\" structure.");
        exit(CHARM_FAILURE);
    ...
    charmf_shc_read_bin(fptr, nmax, shcs2, err);
    fclose(fptr);
    charmf_err_handler(err, 1);
    /* --------------------------------------------------------------------- */
